...
const seedDB = async () => {
  try {
    ...
    // Seed default categories ONLY if collection is completely empty to preserve user data & edits
    const categoryCount = await Category.countDocuments();
    if (categoryCount === 0) {
      await Category.insertMany(seedCategories);
      console.log('Successfully seeded default categories into database!');
    } else {
      console.log(`Categories collection already has ${categoryCount} categories. Preserving user data.`);
    }
